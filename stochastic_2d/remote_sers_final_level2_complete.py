# %% [markdown]
# Final Level 2 — Levels 2.6, 2.7 and 2.8 in one reproducible program
#
...
# Current Level-2.6 physics/configuration from Codex
WAVELENGTH_NM = 633.0
SILVER_RELATIVE_PERMITTIVITY = -15.0 + 1.0j  # current illustrative assumption
# Configurable representative Raman band.  Replace with the measured Raman shift.
RAMAN_SHIFT_CM1 = 1000.0
# No wavelength-resolved Ag table was present in the validated Level-2.6 source.
# Keeping the same value at the Raman wavelength preserves that model explicitly.
SILVER_RELATIVE_PERMITTIVITY_RAMAN = -15.0 + 1.0j
...
OUTPUT_ROOT = (_desktop if _desktop.exists() else Path.cwd()) / "remote_SERS_Final_Level2_results"
...
def run_one_realization(n, seed, out_dir):
    ...
    k_raman = 2 * np.pi / (wavelength_raman_nm * 1e-9)
    alpha_raman = sphere_polarizability(
        aggregate.radii_nm * 1e-9,
        wavelength_raman_nm * 1e-9,
        SILVER_RELATIVE_PERMITTIVITY_RAMAN,
        MEDIUM_RELATIVE_PERMITTIVITY,
    )
    print(
...
def run_study():
    ...
    print("=" * 72)
    print("Remote-SERS final Level 2.6–2.8 complete study")
