...
class FullWaveCalibrationError(RuntimeError):
    """Raised when a correction table cannot be used safely."""


class FullWaveCalibration:
    """Compatibility interface for the validated main-branch calibration API.

    The Level-3B solver and its frozen outputs use the functions below.  This
    small reader remains available so integrating those artifacts does not
    break callers of the pre-existing, bounds-checked interpolation API.
    """

    REQUIRED = (
        "gap_nm", "radius_nm", "wavelength_nm", "polarization_deg",
        "Cparallel_real", "Cparallel_imag", "solver_backend",
        "convergence_status",
    )
    ...
    def __init__(self, path):
        ...
        if missing:
            raise FullWaveCalibrationError(f"missing columns: {sorted(missing)}")
        if not rows:
            raise FullWaveCalibrationError("no executed full-wave calibration rows")
        if any(row["convergence_status"] != "converged" for row in rows):
            raise FullWaveCalibrationError("table contains non-converged full-wave rows")
        if any(row["solver_backend"] in ("", "PENDING", "PDA") for row in rows):
            raise FullWaveCalibrationError("an independent full-wave backend is required")

        axes = [sorted({float(row[key]) for row in rows}) for key in self.REQUIRED[:4]]
        lookup = {
            tuple(float(row[key]) for key in self.REQUIRED[:4]): complex(
                float(row["Cparallel_real"]), float(row["Cparallel_imag"])
            )
            for row in rows
        }
        expected = list(itertools.product(*axes))
        if any(point not in lookup for point in expected):
            raise FullWaveCalibrationError("calibration grid is incomplete")
        values = np.array([lookup[point] for point in expected]).reshape(
            *(len(axis) for axis in axes)
        )
        self.axes = axes
        self.interpolator = RegularGridInterpolator(axes, values, bounds_error=True)

    def get_complex_gap_correction(
        self, gap_nm, radius_nm, wavelength_nm, polarization_deg
    ):
        try:
            ...
            raise FullWaveCalibrationError(
                "query outside calibrated domain; extrapolation forbidden"
            ) from exc
